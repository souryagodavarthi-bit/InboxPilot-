...
# 2. Setup Qwen Client
api_key = "sk-ws-H.PXYPLE.vChx.MEQCIAfHuCvqgljP4Fm4fAK_BydcwsCVpT3ZIu61VbB5Hd3tAiBaf4KKyA-yhrcrdXvblJUxHzU-Tfl8-dM1tRel74M6dg"
try:
    client = OpenAI(
        api_key=api_key,
...
def run_auditor_check(email_text, initial_draft, business_desc):
    auditor_prompt = f"""You are the InboxPilot+ Quality & Personalization Auditor and Polish Agent.
Your job is to inspect the generated response draft to ensure it is 100% personalized to the customer's specific email and complies with company policies:
1. Ensure the draft specifically addresses exact facts, questions, products, or issues mentioned in the customer's original email text.
2. The draft MUST NOT use generic filler phrases like "we received your inquiry regarding your query" or "specialists are checking". It must be a complete, actionable, highly personalized reply.
3. Keep the tone supportive, polite, professional, and empathetic to the customer's mood.
4. Never mention internal database systems, code names, or the fact that you are an AI.

Review the:
- Original Email Content: {email_text}
- Business Profile Context: {business_desc}
- Generated Response Draft: {initial_draft}

Output strictly in this format:
APPROVED: [Yes/No]
FEEDBACK: [If not approved, explain in a single sentence how to rewrite the draft to make it more personalized and specific to their email.]
"""
    try:
        response = client.chat.completions.create(
            model="qwen-plus",
            messages=[
                {"role": "system", "content": "You are a strict quality compliance auditor and polish agent. Output decisions exactly in the requested format."},
        ...
        print(f"Auditor Notice: {e}. Running local polish pass...")
        return True, "Polish verified with local rules."
...
{category_descriptions}
    ...
    ---DRAFT START---
    [Write a clean, friendly, personalized customer response draft. 
    IMPORTANT DRAFT GUIDELINES:
    1. STRICT MANDATE ON TYPOS: NEVER MENTION, REFERENCE, ADDRESS, OR ACKNOWLEDGE CUSTOMER TYPOS, SPELLING ERRORS, OR GRAMMAR MISTAKES. Do not say "despite the typo" or "I understand you meant". Completely ignore any typos/mistakes and continue with the rest of the email directly addressing their intent!
    2. STRICT DASH & FORMATTING BAN: DO NOT include any dashes of any kind (no em-dashes —, en-dashes –, or hyphens/dashes -). Use commas, periods, or new lines instead of dashes to structure your sentences cleanly! DO NOT include any emojis, and DO NOT include any URLs or links in the reply unless explicitly provided by the customer.
    3. NO RESTATING EMAILS: DO NOT restate, quote, or summarize the customer's email text back to them in your response. Jump straight into answering their question or fulfilling their inquiry directly!
    4. Keep the length balanced and concise (not too long, avoid excessive fluff or over-explaining technical details; e.g., if asked about colors, simply list the colors cleanly without exact color codes or numbers).
    5. If the customer's question is brief, vague, or missing key details needed to assist them (such as an order ID or product model), naturally ask polite clarifying questions.
    6. If escalated, apologize for the trouble, acknowledge their history, and mention a manager is reviewing it.]
    {tone_requirement}
    """
...
def send_gmail_reply(access_token, to_email, subject, body, thread_id, in_reply_to_msg_id, bcc_email=None):
    ...
    message['To'] = to_email
    clean_bcc_str = ""
    if bcc_email and str(bcc_email).strip():
        bcc_list = [e.strip() for e in re.split(r'[,;\s]+', str(bcc_email).strip()) if e.strip()]
        if bcc_list:
            clean_bcc_str = ", ".join(bcc_list)
            message['Bcc'] = clean_bcc_str
    ...
    try:
        ...
        if res.status_code == 200:
            bcc_info = f" (BCC: {clean_bcc_str})" if clean_bcc_str else ""
            print(f"Successfully sent reply to {to_email}{bcc_info}")
            return True
        else:
...
def has_sent_prior_response_to_customer(from_header):
    if not from_header:
        return False
    clean_sender = from_header.lower().strip()
    if "<" in clean_sender:
        m = re.search(r'<(.*?)>', clean_sender)
        if m: clean_sender = m.group(1).strip()
    ...
    if os.path.exists(TRIAGE_HISTORY_FILE):
        try:
            with open(TRIAGE_HISTORY_FILE, "r", encoding="utf-8") as f:
                records = json.load(f)
            for r in records:
                r_sender = r.get("sender", "").lower()
                if "<" in r_sender:
                    m2 = re.search(r'<(.*?)>', r_sender)
                    if m2: r_sender = m2.group(1).strip()
                if clean_sender in r_sender and r.get("query_response_sent", False):
                    return True
        except Exception:
            pass
        ...
        if os.path.exists(q_dir):
            for fname in os.listdir(q_dir):
                if fname.startswith("meta_"):
                    try:
                        with open(os.path.join(q_dir, fname), "r", encoding="utf-8") as mf:
                            mdata = json.load(mf)
                            snd = mdata.get("sender_email", "").lower()
                            if clean_sender in snd and mdata.get("query_response_sent", False):
                                return True
                    except Exception:
                        pass
    return False
...
def run_gmail_triage(creds):
    ...
    for msg in messages:
        ...
        if ai_output:
            ...
            ai_says_followup = ("IS_FOLLOWUP: Yes" in classification_part)
            prior_response_sent = has_sent_prior_response_to_customer(from_header)
            
            # Strictly enforce that follow-up only applies if it is a reply to one of our responses
            is_followup = (is_followup or ai_says_followup) and prior_response_sent
            ...
            if category in ("Spam", "Subscription"):
                ...
                cat_bcc = cat_config.get("bcc_email", "") if cat_config else ""
                sent_success = send_gmail_reply(access_token, recipient_email, subject_header, draft_text, thread_id, message_id_header, bcc_email=cat_bcc)
...
def run_local_triage():
    ...
    for filename in files:
        ...
        if ai_output:
            ...
            is_escalated = "ESCALATION: Yes" in classification_part
            ai_says_followup = ("IS_FOLLOWUP: Yes" in classification_part)
            prior_response_sent = has_sent_prior_response_to_customer(customer_name)
...
def main():
    ...
    if creds and "refresh_token" in creds:
        try:
            ...
            print(f"Error during Gmail triage: {e}")
    run_local_triage()
