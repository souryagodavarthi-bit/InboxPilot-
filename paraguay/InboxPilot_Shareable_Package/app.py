...
if not st.session_state.logged_in:
    ...
    with col_c:
        ...
        ">
            ...
            Due to security reasons, please enter your email below. It will be verified within the next 2-3 days, 
        ...
        if st.button("Submit Email for Verification", key="submit_verification_btn", type="secondary", use_container_width=True):
            if not verif_email.strip():
                ...
                try:
                    ...
                    st.success("Thank you! Your email has been saved and will be verified within 2-3 days.")
                except Exception as e:
...
# Main Dashboard View (Logged In)
    ...
    if not st.session_state.tutorial_completed:
        ...
        with col_c:
            ...
            if st.session_state.tutorial_tab == 1:
                ...
                st.markdown('<div style="font-size: 14px; color: #9aa0a6; line-height: 1.6; margin-bottom: 16px;">Provide a brief description of what your business does. This context is injected into the AI model\'s mind to generate highly relevant, professional draft replies.</div>', unsafe_allow_html=True)
            ...
            if st.session_state.tutorial_tab == 7:
                biz_desc = st.text_area(
                    "Tell InboxPilot+ what your business is about so it can reply better!",
                ...
                with col_start:
                    if st.button("Get Started", key="tut_start_btn", type="primary", use_container_width=True):
                        if biz_desc.strip():
                            ...
                            st.error("Please enter a short description of your business to continue.")
            else:
                    ...
                    with col_btn1:
                        if st.button("Clear Support History Cache", key="settings_clear_history_btn", use_container_width=True):
                            ...
                            if os.path.exists(TRIAGE_HISTORY_FILE):
                                try:
                                    os.remove(TRIAGE_HISTORY_FILE)
                                    st.success("History cache successfully cleared!")
                                except Exception as e:
                                    st.error(f"Failed to clear history: {e}")
                            else:
                                st.info("No history cache found.")
    ...
    with tab1:
        ...
        with col1:
            ...
            if gmail_mode:
                num_pending = gmail_unread_count
            else:
            ...
            with st.container():
                ...
                <div class="custom-card">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-size: 11px; color: #9aa0a6; text-transform: uppercase; font-weight: 500; letter-spacing: 0.8px;">Queue Status ({ 'Gmail API' if gmail_mode else 'Local Mode' })</div>
            ...
            # Trigger button
            trigger_clicked = st.button("Trigger AI Triage Engine", type="primary", use_container_width=True)
            
            if trigger_clicked:
                if num_pending == 0:
                    if gmail_mode:
                        st.warning("No new unread emails in Gmail to process.")
                    else:
                        st.warning("No new emails in the Incoming_Emails folder to process.")
                else:
            ...
            # Display raw files / messages in Inbox
            if num_pending > 0:
                if gmail_mode:
                    ...
                    st.markdown("#### Pending Emails List")
            ...
            if gmail_mode:
                ...
                if target_category == "Escalated":
                    filtered_records = [
                        ...
                        if rec.get("escalated", False) and not rec.get("query_response_sent", False)
                    ]
                else:
                    filtered_records = [
                        (idx, rec) for idx, rec in enumerate(history_records)
                        if rec.get("category") == target_category 
                        and not rec.get("query_response_sent", False) 
                ...
                if not filtered_records:
                    ...
                    for display_index, (orig_index, record) in enumerate(filtered_records):
                        ...
                        expander_label = f"{followup_prefix}{escalation_prefix}📩 **{subject_val}** — From: {sender_val} | {date_val} | Confidence: **{confidence_val}**"
                        with st.expander(expander_label):
                                    ...
                                    if not response_val.strip():
                                        ...
                                        try:
                                            ...
                                            if not access_token:
                                                ...
                                                sent = InboxPilot.send_gmail_reply(access_token, to_email, subject_val, response_val, record.get("thread_id"), record.get("message_id"))
                                                if sent:
                                                    record["query_response_sent"] = True
                                                    record["query_response_text"] = response_val
                                                    with open(TRIAGE_HISTORY_FILE, "w") as f:
                                                        json.dump(history_records, f, indent=2)
                                                    st.success("Response sent successfully via Gmail!")
                                                    st.rerun()
                                                else:
                                    ...
                                    if os.path.exists(meta_path):
                                        try:
                                            with open(meta_path, "r", encoding="utf-8") as mf:
                                                ...
                                                if meta_data.get("escalated", False) and not meta_data.get("query_response_sent", False):
                            ...
                            if os.path.exists(meta_path):
                                try:
                                    with open(meta_path, "r", encoding="utf-8") as mf:
                                        ...
                                        query_response_sent = meta_data.get("query_response_sent", False)
                                except:
                                    pass
                            if not is_escalated and not query_response_sent:
                ...
                if not original_emails:
                    ...
                    for index, (email_file, selected_dir) in enumerate(original_emails):
                        ...
                        escalation_prefix = " [ESCALATED] " if is_rec_escalated else ""
                        followup_prefix = "📬 [FOLLOW-UP] " if is_followup else ""
                        expander_label = f"{followup_prefix}{escalation_prefix}📩 **{clean_fn}** — {snippet} | {date_val} | Confidence: **{confidence_val}**"
                        with st.expander(expander_label):
                            unsubscribe_prefs = load_unsubscribe_preferences()
                                ...
                                if st.button("Send Response", key=f"send_query_response_local_{index}", type="primary", use_container_width=True):
                                    ...
                                    if not response_val_local.strip():
                                        ...
                                        try:
                                            ...
                                            st.success("Query response simulated and saved locally.")
                                            st.rerun()
                                        except Exception as e:
    ...
    with tab4:
        ...
        # Enforce maximum 8 categories
        st.markdown(f"### Active Categories: **{total_enabled} / 8**")
        if total_enabled >= 8:
            st.warning("You have reached the maximum limit of 8 active categories. Disable a standard category or delete a custom one to add new ones.")
        else:
            st.success(f"You can add up to {8 - total_enabled} more category/categories.")
        ...
        for index, cat in enumerate(standard_cats):
            col_target1, col_target2 = st.columns([3, 3])
            with col_target1:
                disabled_checkbox = False
                if total_enabled >= 8 and not cat.get("enabled", True):
        ...
        if st.button("Save Standard Category Settings", key="save_standard_settings_btn"):
            ...
            for c in categories:
                if c.get("is_standard", False):
                    ...
                    if old_enabled != new_enabled or old_mode != new_mode:
        ...
        if archived_custom_cats:
            ...
            selected_restore_label = st.selectbox(
                ...
                disabled=(total_enabled >= 8),
                key="restore_category_selectbox"
            )
            if st.button("Restore Category", key="restore_category_btn", disabled=(total_enabled >= 8)):
        ...
        custom_name = st.text_input("Category Name (e.g. Wholesale Inquiry)", placeholder="Wholesale Inquiry", disabled=(total_enabled >= 8))
        custom_desc = st.text_area("Category Description (Explain to the AI when to route emails here)", placeholder="Customer is asking about bulk orders, pricing list, or wholesale partnership terms.", disabled=(total_enabled >= 8))
        ...
        if st.button("Add Category", key="add_custom_category_btn", disabled=(total_enabled >= 8)):
        ...
        st.markdown("Here are the active categories, their AI routing descriptions, and their automation modes:")
        for cat in enabled_categories:
            words = cat['description'].split()
            short_desc = " ".join(words[:8]) + ("..." if len(words) > 8 else "")
            amode = cat.get("automation_mode", "semi_automated")
            mode_badge = "⚡ Automated" if amode == "automated" else ("🤖 Semi-Automated" if amode == "semi_automated" else "✋ Fully Manual")
            st.markdown(f"- **{cat['label']}** (`{mode_badge}`): {short_desc}")
    ...
    with tab5:
        ...
        if not history_records:
            ...
            col_m2.metric("Average Confidence", avg_confidence)
            col_m3.metric("Escalation Rate", escalation_rate)
            col_m4.metric("Sentiment Index", sentiment_score)
            ...
            with col_c2:
                st.markdown("#### Customer Sentiment Spread")
                ...
                st.bar_chart(df_sent, use_container_width=True)
                
            st.markdown("<hr style='border: 0.5px dashed #3c4043; margin: 1.5rem 0;'>", unsafe_allow_html=True)
            st.markdown("#### Support Volume Timeline (Emails Per Day)")
            ...
            st.bar_chart(df_timeline, use_container_width=True)
